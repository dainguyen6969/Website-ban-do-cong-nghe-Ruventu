...
export default function ChiTietCombo() {
  ...
  return <main className="combo-detail-page">
    ...
    <div className="combo-detail-content">
      ...
      <FormCard title="Thành phần combo" className="detail-components-card"><div className="detail-components"><table><thead><tr><th>ẢNH</th><th>TÊN SẢN PHẨM</th><th>TÊN PHIÊN BẢN</th><th>MÃ SẢN PHẨM</th><th>SKU / MÃ VẠCH</th><th>SỐ LƯỢNG / COMBO</th><th>GIÁ COMPONENT</th><th>TỒN CÓ THỂ BÁN</th><th>KHẢ NĂNG TẠO COMBO</th></tr></thead><tbody>{componentPagination.visibleItems.map((item) => { const isBottleneck = item === detail.bottleneck; return <tr key={`${item.sku}-${item.name}`} className={isBottleneck ? 'bottleneck-row' : ''}><td><img src={item.image} alt="" /></td><td><strong>{item.name}</strong></td><td>{item.variant}</td><td><code>{item.productCode}</code></td><td><code>{item.sku}</code></td><td className="detail-number">{item.qty}</td><td><strong>{money(item.price)}</strong></td><td className="detail-stock">{item.stock}</td><td className={isBottleneck ? 'detail-bottleneck' : 'detail-number'}>{item.capacity}{isBottleneck && <small>BOTTLENECK</small>}</td></tr>; })}</tbody></table></div><DetailTablePagination totalItems={detail.components.length} currentPage={componentPagination.currentPage} onPageChange={componentPagination.onPageChange} idPrefix="combo-components" /></FormCard>
