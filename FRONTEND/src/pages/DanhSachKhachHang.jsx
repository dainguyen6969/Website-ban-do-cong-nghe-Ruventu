...
export default function DanhSachKhachHang() {
  ...
  const filteredCustomers = useMemo(() => customers.filter((customer) => {
    ...
    const matchesSearch = !query || [customer.name, customer.email, customer.phone]
  ...
  return (
    <main className="customer-page customer-list-page" role="main">
      <section className="customer-page__hero">
        <div>
          <nav className="customer-page__crumbs" aria-label="Breadcrumb nội dung">
            <span>KHÁCH HÀNG &amp; ĐỐI TÁC</span><span>/</span><strong>KHÁCH HÀNG</strong>
          </nav>
          <h1>DANH SÁCH KHÁCH HÀNG</h1>
          <p>QUẢN LÝ HỒ SƠ VÀ LỊCH SỬ MUA HÀNG KHÁCH HÀNG</p>
        </div>
        <button type="button" className="customer-btn customer-btn--black" aria-label="Thêm khách hàng">
      ...
      <section className="customer-toolbar" aria-label="Tìm kiếm và lọc khách hàng">
        <label className="customer-search" htmlFor="customer-search-input">
        ...
        <FilterDropdown
          ...
          value={statusFilter}
          onSelect={(value) => { setStatusFilter(value); setCurrentPage(1); }}
        />
      </section>
      ...
      <section className="customer-list-content">
          ...
          <table className="customer-table">
            ...
            <tbody>
              {visibleCustomers.map((customer) => (
                ...
                >
                  <td className="customer-code">#{customer.id}</td>
                  <td className="customer-name">{customer.name}</td>
                  <td className={customer.email ? '' : 'customer-muted'}>{customer.email || '—'}</td>
                  <td>{customer.phone}</td>
                  <td><StatusBadge status={customer.status} /></td>
                  <td>
                    <button type="button" className="customer-action-btn" onClick={(event) => { event.stopPropagation(); openCustomer(customer.id); }}>
                      XEM CHI TIẾT
                    </button>
                  </td>
                </tr>
        ...
        <div className="customer-list-footer">
          <p>
            HIỂN THỊ {filteredCustomers.length ? startIndex + 1 : 0}-{Math.min(startIndex + PAGE_SIZE, filteredCustomers.length)} TRÊN TỔNG SỐ {filteredCustomers.length} KHÁCH HÀNG
          </p>
