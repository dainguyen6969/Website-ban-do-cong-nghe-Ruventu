...
export default function ChiTietSanPham() {
  ...
  return <main className="detail-page">
    ...
    {isCombo && <FormCard title="Thành phần sản phẩm (combo)" headerRight={<><Badge>{components.length} LINH KIỆN</Badge><strong className="detail-header-total">TỔNG: {money(componentTotal)}</strong></>} className="detail-card"><table className="detail-table"><thead><tr><th>#</th><th>TÊN SẢN PHẨM</th><th>MÃ SẢN PHẨM</th><th>ĐƠN GIÁ</th><th>SỐ LƯỢNG</th><th>THÀNH TIỀN</th></tr></thead><tbody>{components.map((item, index) => { const line = Number(item.giaNhap || item.donGia || 0) * (item.qty || 1); return <tr key={`${item.id}-${index}`}><td>{index + 1}</td><td className="detail-name">{item.tenSanPham}</td><td><span className="detail-code">{item.maSanPham}</span></td><td>{money(item.giaNhap || item.donGia)}</td><td>{item.qty || 1}</td><td className="detail-red">{money(line)}</td></tr>; })}<tr className="detail-total-row"><td colSpan="5">TỔNG VỐN</td><td className="detail-red">{money(componentTotal)}</td></tr></tbody></table></FormCard>}
    {hasVariantTable && <FormCard title="Thông tin phiên bản" headerRight={<Badge>{variants.length} PHIÊN BẢN</Badge>} className="detail-card"><table className="detail-table"><thead><tr><th>TÊN PHIÊN BẢN</th><th>MÃ SKU</th><th>GIÁ BÁN LẺ</th><th>GIÁ NHẬP</th><th>TỒN ĐẦU KỲ</th><th>TỒN KHO HIỆN TẠI</th></tr></thead><tbody>{variants.map(v => <tr key={v.name}><td className="detail-name">{v.name}</td><td>{v.sku || '—'}</td><td>{money(v.giaBanLe)}</td><td>{money(v.giaNhap)}</td><td>{v.tonDauKy || 0}</td><td>{v.tonKho ?? v.tonDauKy ?? 0}</td></tr>)}</tbody></table></FormCard>}
