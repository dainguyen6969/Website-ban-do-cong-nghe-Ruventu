...
const categoryOptions = [
  'Danh mục', 'CPU', 'VGA', 'Mainboard', 'RAM', 'SSD',
  'PSU', 'Màn hình', 'Tản nhiệt', 'Case', 'PC Build', 'Phụ kiện',
];

const brandOptions = [
  'Thương hiệu', 'Intel', 'AMD', 'ASUS', 'MSI', 'Gigabyte',
  'Corsair', 'Kingston', 'Samsung', 'WD', 'EVGA', 'LG', 'Lian Li',
  'Keychron', 'Ruventu Custom',
];
...
export default function ThemSanPham() {
  ...
  const existingProduct = productId ? getMockProductById(decodeURIComponent(productId)) : null;
  const richTextRef = useRef(null);
  const descriptionInitializedRef = useRef(false);

  // ── General info ──
  const [tenSanPham, setTenSanPham] = useState(() => existingProduct?.tenSanPham || '');
  const [maSanPham, setMaSanPham] = useState(() => existingProduct?.maSanPham || '');
  const [donViTinh, setDonViTinh] = useState(() => existingProduct?.donViTinh || 'Cái');

  // ── Images ──
  const [images, setImages] = useState(() => existingProduct?.hinhAnh ? [{ id: 'existing-image', url: existingProduct.hinhAnh }] : []);

  // ── Description (rich text stored as HTML) ──
  const [moTa, setMoTa] = useState(() => existingProduct?.moTa || '');

  // ── Specs (thông số kỹ thuật) ──
  const [specs, setSpecs] = useState(() => existingProduct?.specs || [
  ...
  // ── Product type toggle ──
  const [productType, setProductType] = useState(() => existingProduct?.phanLoai === 'Theo bộ (Combo)' ? 'combo' : 'single'); // 'single' | 'combo'

  // ── Classification ──
  const [selectedCategory, setSelectedCategory] = useState(() => existingProduct?.danhMuc || 'Danh mục');
  const [selectedBrand, setSelectedBrand] = useState(() => existingProduct?.thuongHieu || 'Thương hiệu');

  // ── Status ──
  const [trangThai, setTrangThai] = useState(() => existingProduct?.trangThaiBan || 'Đang kinh doanh');

  // ── VAT ──
  const [vat, setVat] = useState(() => existingProduct?.vat || '10%');
  ...
  const [attributes, setAttributes] = useState(() => existingProduct?.attributes && !existingProduct.phanLoai.includes('Combo') ? existingProduct.attributes : []);
  const [newAttrName, setNewAttrName] = useState('');
  const [newAttrValues, setNewAttrValues] = useState('');

  // ── Variants ──
  const variants = useMemo(() => generateVariants(attributes), [attributes]);
  const [variantData, setVariantData] = useState(() => Object.fromEntries((existingProduct?.variants || []).map((variant) => [variant.name, variant])));
  ...
  const [comboItems, setComboItems] = useState(() => existingProduct?.comboItems || []);
  const [comboPrice, setComboPrice] = useState(() => existingProduct?.comboPrice || '');
  ...
  const comboSearchResults = useMemo(() => {
    ...
    for (const p of singleProducts) {
      const code = (p.maSanPham || '').toLowerCase();
  ...
  const updateComboQuantity = (id, change) => {
    setComboItems((prev) => prev.map((item) => item.id === id
      ? { ...item, qty: Math.max(1, (item.qty || 1) + change) }
      : item));
  };

  const handleRemoveComboItem = (id) => {
    setComboItems((prev) => prev.filter((p) => p.id !== id));
  };
  ...
  const handleSave = () => {
    ...
    console.log('💾 LƯU SẢN PHẨM THÀNH CÔNG:', newProduct);
      ...
      <div className="add-product-topbar">
        <div className="add-product-topbar__left">
          <nav className="add-product-breadcrumb" aria-label="Breadcrumb">
            ...
            <span className="add-product-breadcrumb__sep" aria-hidden="true">›</span>
            <span className="add-product-breadcrumb__item add-product-breadcrumb__item--current">{existingProduct ? 'CHỈNH SỬA' : 'THÊM MỚI'}</span>
          </nav>
          <h1 className="add-product-title">{existingProduct ? 'CHỈNH SỬA SẢN PHẨM' : 'THÊM SẢN PHẨM MỚI'}</h1>
        </div>
        <div className="add-product-topbar__right">
          ...
          <button type="button" className="btn-add-product btn-add-product--red" onClick={handleSave} id="btn-save-product">
            {existingProduct ? 'CẬP NHẬT SẢN PHẨM' : 'LƯU SẢN PHẨM'}
          </button>
        </div>
      </div>
