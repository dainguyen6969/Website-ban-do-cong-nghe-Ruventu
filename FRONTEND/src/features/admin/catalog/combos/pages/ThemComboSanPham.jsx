// Existing combo form; its custom image layout uses the shared Cloudinary media API.
...
import { uploadImage } from '../../../../media/api/imageUploadApi';
import { createCombo, getComboDetail, getComboFormOptions, updateCombo } from '../api/comboApi';
...
function ComboImageUploader({ images, onChange }) {
  ...
  const addFiles = async (files) => {
    ...
    const added = Array.from(files)
      ...
      .map((file, index) => ({ id: `${Date.now()}-${index}`, url: URL.createObjectURL(file), file, uploading: true }));
    onChange([...images, ...added]);
    const uploaded = await Promise.all(added.map(async (image) => {
      try {
        const result = await uploadImage(image.file, 'products');
        URL.revokeObjectURL(image.url);
        return { ...image, url: result.url, file: null, uploading: false };
      } catch (error) {
        return { ...image, uploading: false, error: error?.message || 'Không thể tải ảnh lên.' };
      }
    }));
    const byId = new Map(uploaded.map((image) => [image.id, image]));
    onChange((current) => current.map((image) => byId.get(image.id) || image));
  };
  ...
  return <div className="combo-images">
    {images.map((image, index) => <div className={`combo-image-item ${index === 0 ? 'main' : ''}`} key={image.id}>
      <div className="combo-image-box" title={image.error || (image.uploading ? 'Đang tải ảnh lên Cloudinary...' : '')}>{index === 0 && <span className="main-badge">ẢNH CHÍNH</span>}<img src={image.url} alt={`Ảnh combo ${index + 1}`} /><button type="button" className="image-remove" onClick={() => remove(image.id)} aria-label="Xóa ảnh">×</button></div>
    ...
    <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp" multiple hidden onChange={(e) => { addFiles(e.target.files); e.target.value = ''; }} />
  </div>;
}
...
export default function ThemComboSanPham() {
  ...
  const [form, setForm] = useState({ name: '', code: '', description: '', category: '', brand: '', retail: '', cost: '', weight: '', active: true, vat: false, vatRate: '10' });
  ...
  useEffect(() => {
    let current = true;
    getComboFormOptions(search).then((options) => {
      if (!current) return;
      setProducts(options.products); setCategories(options.categories); setBrands(options.brands);
    }).catch((requestError) => { if (current) setServerError(requestError.message || 'Không thể tải dữ liệu biểu mẫu.'); });
  ...
  useEffect(() => {
    ...
    getComboDetail(editId).then((combo) => {
      if (!current) return;
      setForm({
        name: combo.name, code: combo.code, description: combo.description,
        category: String(combo.categoryId || ''), brand: String(combo.brandId || ''),
        retail: String(combo.price / 1000), cost: String(combo.cost / 1000),
        weight: String(combo.weight),
        active: !String(combo.status).startsWith('Ng'), vat: combo.vat > 0, vatRate: String(combo.vat || 10),
      });
      setImages(combo.imageItems.map((image, index) => ({ id: image.id || `saved-${index}`, url: image.url })));
      setSpecs(combo.specs); setTags(combo.tags); setComponents(combo.components);
    }).catch((requestError) => { if (current) setServerError(requestError.message || 'Không thể tải combo cần sửa.'); })
  ...
  const addComponent = (product) => {
    ...
    setComponents((prev) => [...prev, { id: product.id, variantId: variant.id || product.id, image: product.hinhAnh || fallbackImage, name: product.tenSanPham, variant: variant.name || 'Mặc định', productCode: product.maSanPham, sku: variant.sku || product.maSanPham, price: Number(variant.giaBanLe || product.giaBanLe || 0), stock: Number(product.coTheBan ?? product.tonKho ?? 0), qty: 1 }]);
  ...
  const submit = async () => {
    const next = {};
    if (images.some((image) => image.uploading)) next.images = 'Ảnh đang được tải lên Cloudinary. Vui lòng chờ hoàn tất.';
    else if (images.some((image) => image.error)) next.images = images.find((image) => image.error).error;
    ...
    setErrors(next);
    if (Object.keys(next).length) { if (next.images) alert(next.images); document.querySelector('.has-error, .component-error')?.scrollIntoView({ behavior: 'smooth', block: 'center' }); return; }
    setServerError('');
    try {
      const payload = { ...form, categoryId: form.category, brandId: form.brand, images, specs, tags, components };
      if (editId) await updateCombo(editId, payload); else await createCombo(payload);
      setSuccess(true); window.setTimeout(() => navigate('/kho-hang/combo-san-pham'), 1400);
    } catch (requestError) {
      setServerError(requestError.message || `Không thể ${editId ? 'cập nhật' : 'thêm'} combo.`);
    }
  };

  return <main className="combo-create-page">
    <div className="combo-page-heading"><div><div className="combo-secondary-breadcrumb"><span>Sản phẩm</span><b>›</b><span>Quản lý kho</span><b>›</b><span>Combo sản phẩm</span><b>›</b><strong>{editId ? 'Chỉnh sửa' : 'Thêm mới'}</strong></div><h1>{editId ? 'SỬA COMBO SẢN PHẨM' : 'THÊM COMBO SẢN PHẨM'}</h1></div><div className="heading-actions"><button className="combo-outline-btn" onClick={() => navigate('/kho-hang/combo-san-pham')}>HỦY</button><button className="combo-primary-btn" disabled={loading} onClick={submit}>{editId ? 'LƯU THAY ĐỔI' : 'LƯU COMBO'}</button></div></div>
    {serverError && <small className="component-error">{serverError}</small>}
