...
public final class TheKhoSpecification {
  ...
  public static Specification<TheKho> build(
    ...
    return (root, query, cb) -> {
      ...
      if (khoHangId != null) {
        predicates.add(cb.equal(root.get("khoHang").get("id"), khoHangId));
      }
      ...
      if (loaiGiaoDich != null) {
        predicates.add(cb.equal(root.get("loaiGiaoDich"), loaiGiaoDich));
      }

      if (tuNgay != null) {
        predicates.add(cb.greaterThanOrEqualTo(root.<LocalDateTime>get("ngayTao"), tuNgay));
      }

      if (denNgayExclusive != null) {
        predicates.add(cb.lessThan(root.<LocalDateTime>get("ngayTao"), denNgayExclusive));
      }
