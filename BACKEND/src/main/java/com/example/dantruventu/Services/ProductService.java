...
public class ProductService {
  ...
  public ProductListResponse getProducts(
    ...
    int currentLimit = limit == null ? DEFAULT_LIMIT : limit;

    validateFilters(giaMin, giaMax, thuongHieuId, danhMucId, currentPage, currentLimit);

    ProductSort productSort = ProductSort.fromValue(sort);

    String normalizedKeyword = normalizeKeyword(keyword);

    Pageable pageable = PageRequest.of(currentPage, currentLimit);
    ...
    List<ProductListResponse.ProductItemResponse> items =
        productPage.getContent().stream().map(this::toProductItemResponse).toList();
  ...
  public ProductDetailResponse getProductDetail(Long productId) {
    ...
    List<ProductDetailResponse.AnhSanPhamResponse> images =
        productDetailRepository.findProductImages(productId).stream()
            .map(this::toProductImageResponse)
            .toList();

    List<ProductDetailResponse.PhienBanResponse> variants =
        productDetailRepository.findActiveProductVariants(productId).stream()
    ...
    if (product.getThuongHieuId() != null) {
      brand =
          ProductDetailResponse.ThuongHieuResponse.builder()
              .id(product.getThuongHieuId())
              .tenThuongHieu(product.getTenThuongHieu())
              .build();
    }
    ...
    return ProductDetailResponse.builder()
        ...
        .danhMuc(
            ProductDetailResponse.DanhMucResponse.builder()
                .id(product.getDanhMucId())
                .tenDanhMuc(product.getTenDanhMuc())
                .build())
        .thuongHieu(brand)
        .loaiSanPham(product.getLoaiSanPham())
        .moTa(product.getMoTa())
        .thongSoKyThuat(product.getThongSoKyThuat())
  ...
  @Transactional(readOnly = true)
  public ProductVariantDetailResponse getProductVariantDetail(Long productId, Long variantId) {
    ...
    ProductDetailProjection.ProductVariantDetailProjection variant =
        productDetailRepository
            .findActiveProductVariantDetail(productId, variantId)
            .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND));

    long actualStock = variant.getTonThucTe() == null ? 0L : variant.getTonThucTe();

    long availableStock = variant.getTonCoTheBan() == null ? 0L : variant.getTonCoTheBan();
  ...
  private ProductListResponse.ProductItemResponse toProductItemResponse(
      ProductListProjection projection) {

    long stock = projection.getTonKhoKhaDung() == null ? 0L : projection.getTonKhoKhaDung();
    ...
    return ProductListResponse.ProductItemResponse.builder()
        ...
        .tenThuongHieu(projection.getTenThuongHieu())
  ...
  private ProductDetailResponse.AnhSanPhamResponse toProductImageResponse(
      ProductDetailProjection.ProductImageProjection projection) {

    return ProductDetailResponse.AnhSanPhamResponse.builder()
        .id(projection.getId())
  ...
  private ProductDetailResponse.PhienBanResponse toProductVariantResponse(
      ProductDetailProjection.ProductVariantProjection projection) {

    long stock = projection.getTonCoTheBan() == null ? 0L : projection.getTonCoTheBan();

    return ProductDetailResponse.PhienBanResponse.builder()
        .id(projection.getId())
