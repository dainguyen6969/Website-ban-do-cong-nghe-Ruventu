...
public class LoginService {
  ...
  public LoginResult login(LoginRequest request) {
    ...
    Optional<NguoiDung> optionalUser =
        nguoiDungRepository.findByEmailOrSoDienThoai(taiKhoan, taiKhoan);
    ...
    boolean ghiNhoDangNhap = Boolean.TRUE.equals(request.getGhiNhoDangNhap());

    AuthTokenService.TokenPair tokenPair = authTokenService.generateTokenPair(user, ghiNhoDangNhap);
    ...
    LoginResponse.UserData userData =
        LoginResponse.UserData.builder()
            ...
            .vaiTroId(user.getVaiTro() != null ? user.getVaiTro().getId() : null)
            .build();
    ...
    LoginResponse response =
        LoginResponse.builder().status(200).message("Đăng nhập thành công").data(loginData).build();
