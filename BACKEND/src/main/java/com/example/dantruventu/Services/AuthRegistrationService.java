...
public class AuthRegistrationService {
  ...
  public RegisterResponse register(RegisterRequest request) {
    ...
    return RegisterResponse.builder().thoiHanOtp(OTP_EXPIRE_SECONDS).build();
  }

  public ResendOtpResponse resendOtp(ResendOtpRequest request) {

    PendingRegistration pending = findPendingRegistration(request.getTaiKhoan());
    ...
    if (now.isBefore(pending.lastSentAt.plusSeconds(RESEND_COOLDOWN_SECONDS))) {
  ...
  public VerifyOtpResponse verifyOtp(VerifyOtpRequest request) {
    ...
    return VerifyOtpResponse.builder().userId(savedUser.getId()).build();
  }
  ...
  private PendingRegistration findPendingRegistration(String taiKhoan) {
    ...
    PendingRegistration byEmail = pendingRegistrations.get(value.toLowerCase());
    ...
    return pendingRegistrations.values().stream()
        .filter(pending -> pending.soDienThoai.equals(value))
        .findFirst()
        .orElse(null);
  }
