...
public class AdminInventoryController {
    ...
    public ApiResponse<AdminInventoryListResponse> getItems(
            ...
            String keyword,
            @RequestParam(name = "loai_doi_tuong", defaultValue = "ALL")
            String loaiDoiTuong,
            @RequestParam(name = "kho_hang_id", required = false)
            Long khoHangId,
            @RequestParam(name = "chi_canh_bao", defaultValue = "false")
            boolean chiCanhBao,
            @RequestParam(name = "page", defaultValue = "0")
            int page,
            @RequestParam(name = "limit", defaultValue = "20")
            int limit,
            @RequestParam(name = "sort", defaultValue = "ten_hien_thi,asc")
            String sort) {
    ...
    public ApiResponse<AdminInventoryDetailResponse> getDetail(
            ...
            @RequestParam(name = "loai_doi_tuong", required = false)
            String loaiDoiTuong) {
        ...
        return ApiResponse.<AdminInventoryDetailResponse>builder()
                ...
                .data(
                        adminInventoryService.getDetail(id, loaiDoiTuong))
                .build();
    }
    ...
    public ApiResponse<AdminInventoryLedgerListResponse> getLedger(
            @RequestParam(name = "kho_hang_id", required = false)
            Long khoHangId,
            @RequestParam(name = "phien_ban_id", required = false)
            Long phienBanId,
            @RequestParam(name = "loai_giao_dich", required = false)
            String loaiGiaoDich,
